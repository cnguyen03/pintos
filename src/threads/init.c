...
int pintos_init(void) {
  ...
  if (*argv != NULL) {
    ...
    // Run interactively if no argument given
    char input[256];
    while (true) {
      int i = 0;
      printf("ICS143A>\n");
      while (true) {  // Get user input
        char c = input_getc();
        if (c ==
            '\n') {  // newline encountered, finish gathering input by breaking
          break;
        }
        input[i++] = c;
        // if (input_full) { // input buffer full
      ...
      printf(input);
      printf("\n");
      if (strcmp(input, "whoami") == 0) {  // whoami command
        printf("\nCalvin\n");
